...
<div class="card">
    <div class="card-header">
        ...
        <div class="card white box-content">
            <div class="card-body">
                <form action="{{ route('diskon.update', $kupon->id) }}" method="post" enctype="multipart/form-data">
                    @method('put')
                    @csrf
                    <div class="form-group">
                        <label>Kode Kupon</label>
                        <input type="text" name="kode" value="{{ old('kode', $kupon->kode) }}"
                    ...
                    <div class="form-group">
                        <label>Diskon (%)</label>
                        <input type="number" name="diskon" value="{{ old('diskon', $kupon->diskon) }}"
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" name="tanggal_kadaluarsa" value="{{ old('tanggal_kadaluarsa', $kupon->tanggal_kadaluarsa) }}"
                    ...
                    <div class="form-group">
                        ...
                        <select name="status" class="form-control @error('status') is-invalid @enderror">
                            <option value="1" {{ old('status', $kupon->status) == '1' ? 'selected' : '' }}>Aktif</option>
                            <option value="0" {{ old('status', $kupon->status) == '0' ? 'selected' : '' }}>Non-aktif</option>
                        </select>
                        @error('status')
                            <span class="invalid-feedback alert-danger" role="alert">{{ $message }}</span>
                        @enderror
